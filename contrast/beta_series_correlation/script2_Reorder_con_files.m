...
base_dir = [DME_dir '/GLM/beta_series_correlation/'];
...
names_contrast = {'SWWS_D' 'SWWS_M' 'SWWS_E' ...
...
%for s = 1:1
    ...
    sub_dir = [first_level subjects{s} '/betaseries'];
    cd(sub_dir);
    ...
    for n=1:length(contrast_files)
        ...
        if strncmp(contrast_files{n}, 'con_zfcmap_sphere5mm_left', 25) % compare n = 25 first letters
            % create index for names_contrast variable
            n_con=n;
            name_contrast=names_contrast{n_con};
        else
            % create index for names_contrast variable
            n_con=n-length(names_contrast);
            name_contrast=names_contrast{n_con};
        end
