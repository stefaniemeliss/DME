...
save_dir = [DME_dir '/scripts/contrast/parametric_modulation/pm_val/'];
cd(save_dir);

for s = 1 :length(subjects)
    
    
    load DME_FirstLevel_Contrast_pm_val.mat %random template
    
    SUBJ_ID = subjects{s};
    
    % set up model directory
    moddir = ['/storage/shared/research/cinn/2018/MAGMOT/DME/GLM/parametric_modulation/pm_val/1st-level/' SUBJ_ID '/'];
    cd(moddir);
    
    % select spmmat
    matlabbatch{1,1}.spm.stats.con.spmmat{1,1} = [moddir 'SPM.mat'];
    
    % get in the DME_contrasts_pm_val_*.mat file containing the object consess
    ...
    if ismember(SUBJ_ID, subjects_full)
        load DME_contrasts_pm_val_Full.mat
    elseif ismember(SUBJ_ID, subjects_noError)
        load DME_contrasts_pm_val_NoError.mat
    elseif strcmp(subjects{s}, subject_special)
        load DME_contrasts_pm_val_8_10.mat
    end
    ...
    eval(['save DME_FirstLevel_Contrast_pm_val_' SUBJ_ID '.mat matlabbatch']);
