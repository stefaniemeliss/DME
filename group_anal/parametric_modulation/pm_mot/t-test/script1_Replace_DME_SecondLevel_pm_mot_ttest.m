...
save_dir = [DME_dir '/scripts/group_anal/parametric_modulation/pm_mot/t-test'];
mkdir(save_dir);
cd(save_dir);

% load template
load DME_SecondLevel_pm_mot_ttest.mat


% define name of models and contrast folders
contrast_dir = [DME_dir '/GLM/parametric_modulation/pm_mot/contrasts'];
...
for g = 1:length(group) %iterates over all contrasts
    ...
    for i = 1:length(contrast) %iterates over all contrasts
        ...
        model_dir = [DME_dir '/GLM/parametric_modulation/pm_mot/2nd-level/t-test/' contrast{i} '/' group{g} '/'];
        mkdir(model_dir)        
        ...
        eval(['save DME_SecondLevel_pm_mot_ttest_' contrast{i} '_' group{g}  '.mat matlabbatch']);
    end
end
