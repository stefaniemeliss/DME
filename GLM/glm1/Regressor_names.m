...
load_dir = [DME_dir '/scripts/GLM/glm1/Regress_m1/'];
...
for s = 1:length(subjects)
    ...
    eval(['load ' subjects{s} '_regress_m1_s1.mat']);
    A = [];
    A = names
    
    % get information on regressors for second session
    eval(['load ' subjects{s} '_regress_m1_s2.mat']);
    B = [];
    B = names
 ...
 cd(save_dir);
 eval(['save DME_names_regress_m1.mat reg_names']);