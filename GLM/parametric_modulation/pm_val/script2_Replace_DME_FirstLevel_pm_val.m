...
save_dir = [DME_dir '/scripts/GLM/parametric_modulation/pm_val/'];
...
for s = 1:length(subjects)
    ...
    % load matlab batch
    cd(save_dir)
    load DME_FirstLevel_pm_val.mat
    ...
    moddir = [DME_dir '/GLM/parametric_modulation/pm_val/1st-level/' SUBJ_ID '/'];
    mkdir(moddir);
    cd(moddir);
    % specify data_dir
    ...
    for i = 1:2 % for both sessions
        ...
        matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,i).multi{1,1} = [DME_dir '/scripts/GLM/parametric_modulation/pm_val/Regress_pm_val/' SUBJ_ID '_regress_pm_val_s' num2str(i) '.mat'];
    ...
    cd(save_dir);
    eval(['save DME_FirstLevel_pm_val_' SUBJ_ID '.mat matlabbatch']);
