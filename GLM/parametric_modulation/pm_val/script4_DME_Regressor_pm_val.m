...
load_dir = [DME_dir '/scripts/GLM/parametric_modulation/pm_val/Regress_pm_val/'];
...
for s = 1:length(subjects)
    ...
    eval(['load ' subjects{s} '_regress_pm_val_s1.mat']);
    A = [];
    A = names;
    
    % get information on regressors for second session
    eval(['load ' subjects{s} '_regress_pm_val_s2.mat']);
    B = [];
    B = names;
 ...
 cd(save_dir);
 eval(['save DME_names_regress_pm_val.mat reg_names']);
